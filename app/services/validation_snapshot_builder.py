...
from ..schemas.request import (
    SupplierValidationBatchRequest,
    SupplierValidationRecordRequest,
...
class ValidationSnapshotBuilder:
    ...
    def build_batch_snapshot(
        ...
        *,
        ...
        technical_status = (
            ...
            if any(record.technical_status == TechnicalStatus.RECEIVED for record in processed_records)
        ...
        return ValidationBatchResponse(
            ...
            segment_name=None,
            callback_phone=None,
            callback_contact_name=None,
            source=payload.source.value,
            batch_status=(BatchStatus.RECEIVED if technical_status == TechnicalStatus.RECEIVED else BatchStatus.COMPLETED),
            processed_at=snapshot_time,
            created_at=snapshot_time,
            updated_at=snapshot_time,
            finished_at=(snapshot_time if technical_status == TechnicalStatus.COMPLETED else None),
    ...
    def build_supplier_batch_snapshot(
        self,
        ...
        *,
        ...
        snapshot_time = datetime.now(timezone.utc)
        processed_records = [
            self._build_supplier_record_snapshot(record, segment_name=payload.segment_name, callback_phone=payload.callback_phone)
    ...
    def _build_record_snapshot(self, record: ValidationRecordRequest) -> ValidationRecordResponse:
        ...
        if not cnpj_exists:
            return self._build_failed_record(
                ...
                observation="CNPJ nao localizado na base oficial de empresas consultada via BrasilAPI.",
            )

        official_registry_email = self.official_company_registry.find_contact_email(cnpj=normalized_cnpj)
    ...
    def _build_supplier_record_snapshot(
        self,
        record: SupplierValidationRecordRequest,
        *,
        segment_name: str,
        callback_phone: str,
    ) -> ValidationRecordResponse:
        ...
        if normalized_phone is None:
            return ValidationRecordResponse(
                external_id=record.external_id,
                client_name=record.supplier_name,
                cnpj_original="",
                ...
                phone_confirmed=False,
                confirmation_source=None,
                supplier_validation=SupplierValidationDetails(
                    segment_name=segment_name,
                    callback_phone_informed=callback_phone,
                ),
                final_status=FinalStatus.VALIDATION_FAILED,
                observation="Telefone invalido apos a padronizacao para validacao de fornecedor.",
            )
        ...
        return ValidationRecordResponse(
            ...
            final_status=FinalStatus.PROCESSING,
            observation="Registro de fornecedor recebido e aguardando ligacao de qualificacao.",
        )
    ...
    def _build_summary(self, records: list[ValidationRecordResponse]) -> ValidationBatchSummary:
        return ValidationBatchSummary(
            ready_for_call=sum(record.business_status == BusinessStatus.READY_FOR_CALL for record in records),
            ready_for_retry_call=sum(record.business_status == BusinessStatus.READY_FOR_RETRY_CALL for record in records),
            validation_failed=sum(record.final_status == FinalStatus.VALIDATION_FAILED for record in records),
            invalid_phone=sum(record.business_status == BusinessStatus.INVALID_PHONE for record in records),
            cnpj_not_found=sum(record.business_status == BusinessStatus.CNPJ_NOT_FOUND for record in records),
            processing=sum(record.final_status == FinalStatus.PROCESSING for record in records),
            pending_records=sum(record.final_status == FinalStatus.PROCESSING for record in records),
            validated_records=sum(record.final_status == FinalStatus.VALIDATED for record in records),
            failed_records=sum(record.final_status == FinalStatus.VALIDATION_FAILED for record in records),
            confirmed_by_call=sum(record.business_status == BusinessStatus.CONFIRMED_BY_CALL for record in records),
            confirmed_by_whatsapp=sum(record.business_status == BusinessStatus.CONFIRMED_BY_WHATSAPP for record in records),
            waiting_whatsapp_reply=sum(record.business_status == BusinessStatus.WAITING_WHATSAPP_REPLY for record in records),
            confirmed_by_email=sum(record.business_status == BusinessStatus.CONFIRMED_BY_EMAIL for record in records),
            waiting_email_reply=sum(record.business_status == BusinessStatus.WAITING_EMAIL_REPLY for record in records),
        )
